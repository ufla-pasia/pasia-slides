...
No jogo _Invasão Alienígena_, o jogador controla uma nave que apareceno centro inferir da tela.
...
O jogo termina quando o jogador perder três naves.
:::
...
- Os atributos da classe são criados dinamicamente.
  - Eles não são previamente declaradas como em Java.
...
- Verificamo se um dos eventos é uma tecla pressionada.
...
- Na classe `InvasaoAlien`, vamos chamar atualizar a cada passo do loop do jogo.
...
2. **Atenção**: `print` deve ser removido depois de testar! Afeta o desempenho.
...
Vamos agora adicionar os inimgos do jogo: os alienígenas.
...
Isso tem relação com o _duck typing_ do Python.
...
- O método `verificar_borda()` será usado na classe `InvasaoAlien`.
  - Se um dos aliens atingir uma borda, o esquadrão deve descer e mudar de direção.
...
- `groupcollide()` detecta colisões entre dois grupos.
...
- `spritecollideany()` verifica se um `Sprite` colidiu com qualquer sprite em um grupo.
- Por enquanto estamos apenas testando a colisão.
- Pode ser interessante mudar a velocidade dos aliens para testar. 
...
Se estiver usando um _linter_ como o `pylance` você verá um warning em `self.nave`.
